...
from database.models import db, Question, Choice, TestResult, ProfilePattern, Stage2Trait, User
import random
import json
...
def test():
    # 固定問題2問を取得
    fixed_questions = Question.query.filter_by(is_fixed=True).all()
    
    if len(fixed_questions) < 2:
        flash('固定問題が不足しています。管理者に連絡してください。', 'danger')
        return redirect(url_for('index'))
    
    # 固定問題2問を選択
    selected_fixed = fixed_questions[:2]
    
    # 通常問題から22問をランダム選択
    normal_questions = Question.query.filter_by(is_fixed=False).all()
    if len(normal_questions) < 22:
        flash('通常問題が不足しています。', 'danger')
        return redirect(url_for('index'))
    
    selected_normal = random.sample(normal_questions, 22)
    
    # 固定問題2問 + 通常問題22問 = 計24問
    selected_questions = selected_fixed + selected_normal
    
    # 質問順序をシャッフル（固定問題がランダムな位置に配置される）
    random.shuffle(selected_questions)
    
    # セッションに質問IDを保存
    session['question_ids'] = [q.id for q in selected_questions]
    session['fixed_question_ids'] = [q.id for q in selected_fixed]
...
def submit_test():
    ...
    fixed_question_ids = session.get('fixed_question_ids', [])
    ...
    # 各DISCタイプのカウント（N値も含む）
    most_counts = {'D': 0, 'I': 0, 'S': 0, 'C': 0, 'N': 0}
    least_counts = {'D': 0, 'I': 0, 'S': 0, 'C': 0, 'N': 0}
    
    # 通常問題の集計
    for q_id, choice_id in most_choices.items():
        if q_id not in fixed_question_ids:  # 通常問題のみ
    ...
    # 固定問題の特殊処理
    for q_id in fixed_question_ids:
        if q_id in most_choices:
            most_choice = Choice.query.get(most_choices[q_id])
            if most_choice:
                if most_choice.disc_type == 'N':
                    # N型選択肢が「最も当てはまる」で選択された場合
                    most_counts['N'] += 1
                elif most_choice.is_most_applicable:
                    # 特殊処理が設定されている場合
                    most_counts[most_choice.is_most_applicable] += 1
                else:
                    # 通常のDISC型として処理
                    most_counts[most_choice.disc_type] += 1
        
        if q_id in least_choices:
            least_choice = Choice.query.get(least_choices[q_id])
            if least_choice:
                if least_choice.disc_type == 'N':
                    # N型選択肢が「最も当てはまらない」で選択された場合
                    least_counts['N'] += 1
                elif least_choice.is_least_applicable:
                    # 特殊処理：「最も当てはまらない」で選択された時にN値を加算
                    least_counts['N'] += 1
                else:
                    # 通常のDISC型として処理
                    least_counts[least_choice.disc_type] += 1
    ...
    diff_counts = {
        ...
        'N': most_counts['N'] - least_counts['N']
    }
    
    # セグメント番号の計算（N値は除外）
    segment_1 = calculate_segments({k: v for k, v in most_counts.items() if k != 'N'})
    segment_2 = calculate_segments({k: v for k, v in least_counts.items() if k != 'N'})
    segment_3 = calculate_segments({k: v for k, v in diff_counts.items() if k != 'N'})
    ...
    # ステージIIの結果計算（N値は除外）
    stage_2_result = calculate_stage_2_result({k: v for k, v in diff_counts.items() if k != 'N'})
...
def team_data(team_id):
    ...
    avg_n = sum(r.graph_3_n for r in team_results) / len(team_results) if team_results else 0
